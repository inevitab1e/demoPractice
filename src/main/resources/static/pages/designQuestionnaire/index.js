...
const handleAddQuestion = async (type) => {
    ...
    await $.ajax({
      url: API_BASE_URL + '/question/insert',
      type: 'POST',
...
const handleEditQuestion = (problemIndex) =>{
  ...
  let params = {
    ...
    questionName: problem[problemIndex].problemName,
    questionOption: JSON.stringify(problem[problemIndex].option),
...
const onAddQuestion = async (type) => {
  ...
  $('#problem').append(ele)
  problem.push({ problemName: '', mustAnswer: true, option: [{}], id: addedQuestionId })
